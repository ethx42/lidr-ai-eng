...
- [ ] 9.3 Write prompt `v2` (language rule, evidence self-check, per-surface frontend tasks) and, after its live run, `v3` (evidence stays in the transcript's language under a different output language), bumping `PROMPT_VERSION` each time; verify the pinned hash and prompt tests, then a live `make eval` on `openai/gpt-4o-mini` and a new baseline
