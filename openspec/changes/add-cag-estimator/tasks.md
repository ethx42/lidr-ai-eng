...
- [ ] 3.3 Implement `services/rendering.py` (markdown from enriched breakdown); verify test that rendered total equals computed total

## 4. CAG prompt and context

- [ ] 4.1 Write three typed reference estimations in `context/examples.py` (small/medium/large, different domains); verify each validates against `EstimationBreakdown`
...
- [ ] 5.3 Implement OpenAI adapter (`responses.parse`, usage incl. cached tokens, incomplete/None → `InvalidModelOutput`, SDK error mapping); verify unit tests with mocked SDK client
- [ ] 5.4 Implement Anthropic adapter (`messages.parse`, cached system block, refusal/max_tokens → `InvalidModelOutput`, SDK error mapping); verify unit tests with mocked SDK client
- [ ] 5.5 Implement provider factory and structured `llm_call` logging without transcript content; verify tests for selection and a log record that excludes transcript text

## 6. Service and HTTP API

- [ ] 6.1 Implement `EstimationService` in `services/llm_service.py` (prompt → provider → totals → render → response); verify unit tests with `FakeProvider`
...
- [ ] 7.2 Implement `evals/run_eval.py` (checks, summary table, versioned JSON report); verify with `FakeProvider` in a test and one real `make eval` run producing a report
