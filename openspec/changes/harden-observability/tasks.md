...
- [ ] 2.1 Add failing tests: `LLMError` exposes `cause`/`upstream_status` from a chained exception and prefers an explicit `reason`. An Anthropic `400` (mock transport, error message `"secret upstream detail"`) produces an `llm_call` record with `outcome=upstream_error`, `upstream_status=400`, `cause=BadRequestError`, and no `"secret upstream detail"`. An Anthropic `stop_reason: "max_tokens"` and an OpenAI `incomplete` / `max_output_tokens` response log `cause` `stop_reason:max_tokens` / `incomplete:max_output_tokens`. Successful records carry neither field
- [ ] 2.2 Implement D2 (`reason` keyword and the properties on `LLMError`, provider reasons on invalid-output paths, the optional fields in `log_llm_call`, the service wiring). Verify the 2.1 tests pass
- [ ] 2.3 Extend `test_unhandled_error_returns_json_500_with_request_id`: the captured `unhandled_error` record, formatted by `JsonFormatter`, has `exc_type == "RuntimeError"`, a `stack` entry naming `explode`, and no `"secret internals"`. Implement D3. Verify the test passes and `make check` exits 0
