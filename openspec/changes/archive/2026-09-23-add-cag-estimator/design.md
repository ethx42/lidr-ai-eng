...
- **v4:** the v3 live run fixed `05-explicit-language` but regressed the default: all three English transcripts came back with Spanish narrative (score 0.9362), most likely primed by v3's Spanish example and the reminder in every user message. v4 keeps v2's system prompt byte for byte and sends the evidence reminder in the user message only when `output_language` is explicit, the one case where models translate quotes. *Expected eval impact:* v2's default-language results with grounding recovered on `05-explicit-language`.
