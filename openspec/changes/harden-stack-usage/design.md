...
### D5. Guarding runtime imports with a lockfile-closure test
A test walks `app/**/*.py` with `ast` and collects top-level third-party module names. It maps them to distributions with `importlib.metadata.packages_distributions()` and checks each is in the runtime closure computed from `uv.lock` (the root package's `dependencies`, followed transitively and ignoring `dev-dependencies`). Only the standard library is used (`ast`, `tomllib`, `importlib.metadata`).
- *Why a closure, not direct declaration:* `pydantic` and `starlette` come in through `fastapi`, and forcing direct declarations for them is out of scope. The failure this guards against is "not installed without dev deps".
- *Rejected:* `uv sync --no-dev` + import smoke test in CI. It is slower, it needs a second environment, and the local `make check` would not catch the problem.
