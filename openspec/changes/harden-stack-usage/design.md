...
### D7. Gate strictness
- **pytest:** `[tool.pytest.ini_options]` becomes a native `[tool.pytest]` table with `strict = true` (strict config, markers, xfail, parametrize ids) and `asyncio_default_fixture_loop_scope = "function"`. Verification: `pytest` shows no warnings and does not fail on unknown keys (strict config catches those). Applied: pytest-asyncio 1.4.0 reads the native table (its header reports `asyncio_default_fixture_loop_scope=function`), and an unknown key aborts the run. One warning remains: a third-party `DeprecationWarning` from Starlette 1.6.0's `TestClient` using the `anyio.abc.BlockingPortal` alias. It is left visible, not filtered, because the fix belongs upstream.
- **ruff:** `extend-select`, so the 0.16 default set is extended rather than replaced. `target-version` is removed because it is inferred from `requires-python`. New findings are fixed in code. A per-file ignore is used only where a rule conflicts with an intended pattern, and the reason is given inline. Applied: three findings, two `ISC004` (implicit concatenation inside a list, now parenthesized) and one `PLR0402`. No ignores were added.
- **mypy:** `warn_unreachable = true`, `enable_error_code = ["ignore-without-code", "redundant-expr", "truthy-bool", "possibly-undefined"]`, and `[tool.pydantic-mypy] init_forbid_extra = true, init_typed = true, warn_required_dynamic_aliases = true`. New findings are fixed in code, not suppressed. Applied: `app/` had no findings.
